...
import { Trash2, Edit2, Clock, Check, Calendar } from 'lucide-react';
...
export const TaskItem = React.memo(({ task, onEdit }: TaskItemProps) => {
  ...
  // Design Doc Section 4.A - Color Coding
  const priorityStyles = {
    low: { bg: 'bg-status-success/10', text: 'text-status-success', border: 'border-status-success/20' },
    ...
    urgent: { bg: 'bg-status-danger', text: 'text-white', border: 'border-status-danger' },
  };
  ...
  return (
    <div className={cn(
      ...
      "bg-background-paper dark:bg-background-paper-dark", // Card background
      task.done 
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-start mb-2">
            ...
            <span className={cn(
              "px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-lg border shrink-0", 
          ...
          <div className="flex items-center gap-3 text-xs font-semibold text-foreground-muted dark:text-gray-500 mt-3">
             <div className="flex items-center gap-1.5 bg-background dark:bg-background-dark px-2.5 py-1.5 rounded-md border border-gray-100 dark:border-gray-800">
                ...
                {/* Visualizing Time Block Logic */}
                <span>{task.expectedTime}m Block</span> 
             </div>
             ...
             {task.points && (
               <span className="text-brand-600 dark:text-brand-400 font-bold ml-auto bg-brand-50 dark:bg-brand-900/20 px-2 py-1 rounded-md">
                 +{task.points} pts
               </span>
             )}
          </div>
