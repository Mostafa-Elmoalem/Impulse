...
import { Trash2, Edit2, Clock, Check, Calendar, Zap, AlignLeft, Flag } from 'lucide-react';
...
export const TaskItem = React.memo(({ task, onEdit }: TaskItemProps) => {
  ...
  // --- PROFESSIONAL STYLE LOGIC ---
  // High Contrast White Card + Vivid Left Border Strip
  const getPriorityStyles = () => {
    ...
    switch (task.priority) {
      case 'urgent':
        return { 
          border: 'border-l-red-600', // Strong Red Strip
          bg: 'bg-white', 
      ...
      case 'high':
        return { 
          border: 'border-l-orange-500', // Strong Orange Strip
          bg: 'bg-white', 
          badge: 'text-orange-700 bg-orange-50 border border-orange-100',
          iconColor: 'text-orange-500'
        };
      case 'medium':
        return { 
          border: 'border-l-blue-500', // Strong Blue Strip
          bg: 'bg-white', 
      ...
      default:
        return { 
          border: 'border-l-emerald-500', // Strong Green Strip
          bg: 'bg-white', 
  ...
  return (
    <>
      <div className={cn(
        ...
        "border border-gray-200 dark:border-gray-800", // Crisp outer border
        "border-l-[4px]", // The Status Indicator Strip
        styles.border, // Applies the color strip
        styles.bg, // Background
        "dark:bg-background-paper-dark dark:hover:bg-gray-800/60", // Dark mode override
        "shadow-sm hover:shadow-md hover:-translate-y-[1px]", // Lift effect
        ...
        <div className="flex items-start gap-3.5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex justify-between items-start mb-1.5">
              <h3 className={cn(
                "text-sm sm:text-base font-semibold transition-all truncate pr-3 text-gray-900 dark:text-gray-100",
            ...
            {task.type === 'project' && task.subTasks && task.subTasks.length > 0 && (
               <div className="mb-3 pl-3 border-l-2 border-gray-200 dark:border-gray-700 space-y-1">
            ...
            {/* Footer Meta Info - High Contrast Icons */}
            <div className="flex items-center gap-4 mt-2 text-xs font-medium text-gray-500 dark:text-gray-400">
               <div className="flex items-center gap-1.5">
                  <Clock size={14} className="text-gray-400" />
                  <span className="text-gray-700 dark:text-gray-300">{task.expectedTime}m</span> 
               </div>
               
               <div className="flex items-center gap-1.5">
                  <Calendar size={14} className="text-gray-400" />
                  <span className="text-gray-700 dark:text-gray-300">{format(new Date(task.day), 'MMM d')}</span>
               </div>
