...
import { Trash2, Edit2, Clock, Check, Zap } from 'lucide-react';
import { Task } from '../types';
import { useToggleTaskDone, useDeleteTask } from '../hooks/useTasks';
import { useUIStore } from '@/shared/stores/useUIStore'; // Import Store
...
export const TaskItem = React.memo(({ task, onEdit }: TaskItemProps) => {
  ...
  const setTaskToComplete = useUIStore((state) => state.setTaskToComplete); // Get Action

  const handleCheckClick = () => {
    if (!task.done) {
      // ✅ بدلاً من فتح النافذة هنا، نرسل المهمة للمتجر العالمي
      setTaskToComplete(task);
    } else {
      // Uncheck is simple undo
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-2 text-xs font-medium text-gray-500 dark:text-gray-400">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-gray-700 dark:text-gray-300 font-mono tracking-tight">
                  {task.done && task.actualTime ? `${task.actualTime}m (Actual)` : timeDisplay}
                </span> 
              </div>
              
              {task.done && task.points > 0 && (
                <span className="ml-auto flex items-center gap-1 font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 px-1.5 py-0.5 rounded border border-amber-100 dark:border-amber-900/30">
